...
var convert = function (req, res, type) {
    var items = req.body;
    var promises = _.chain(items).each(function(o){
        extendProps(o, type);
    }).map(generateImage).value();

    Q.all(promises).done(function (results) {
        if (/html/i.test(type)) returnAsHtml(res, results);
        else if (/svg/i.test(type)) returnAsSvg(res, results);
...
var extendProps = function (o, type) {
    var d = new Date();
    _.extend(o, {
...
var returnAsSvg = function (res, results) {
    var images = '';
    var _concat = function(buf){ images += buf; };
