...
(function () {
            ...
            if (system.args[i].charAt(0) === '-') {
                ...
                if (key === 'infile' || key === 'callback' || key === 'dataoptions' || key === 'globaloptions' || key === 'customcode') {
                    ...
                    try {
                        ...
                        console.log(JSON.stringify({ error: 'Error: cannot find file, ' + arg }));
                        phantom.exit();
                    }
                } else {
    ...
    render = function (params, exitCallback) {
        ...
        renderSVG = function (svg) {
            ...
            try {
                ...
                console.log(JSON.stringify({ error: 'ERROR: While rendering, ' + e.toString() }));
            }
        };
        ...
        if (params.length < 1) {
            ...
            input = JSON.stringify(params.infile);
    ...
    startServer = function (host, port) {
        ...
        // run the phantom server for a max ten minutes at a time
        setTimeout(function () {
            var date = new Date();
            console.log(JSON.stringify({ log: 'SERVER KILLED @ ' + date.toString() }));
            phantom.exit();
            server.close();
        }, tenMinutes);
        ...
        // console.log('OK, PhantomJS IMAGE SERVER IS READY');
    };
    ...
    if (!fs.exists(config.tmpDir)) {
        try{
            ...
            console.log(JSON.stringify({ error: 'ERROR: Cannot make temp directory' }));
        }
    }


    if (args.host !== undefined && args.port !== undefined) {
        // start phantom from here before you start
